...
function createModal(scraped, screenshotDataUrl) {
  ...
  // Only warn when the scrape really came up empty — no stem, no selected,
  // no correct. If any of those is set, the boxes below will tell the user
  // what was captured and they can judge for themselves.
  const totallyEmpty =
    !(scraped.questionStem || "").trim() &&
    !scraped.selectedAnswer &&
    !scraped.correctAnswer;
  const warningHtml = totallyEmpty
    ? `<div class="mn-modal-warning">
         ⚠️ <strong>No question content detected.</strong> The scraper
         didn't find a question stem or answers. Are you on the item-review
         screen? If you save anyway, the entry will be mostly empty.
       </div>`
    : "";
